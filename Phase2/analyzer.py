...
class APIConnections:
    """
    This class uses the bearer token for authentication, and
    checks the connections to both the twitter API and the
    Google NLP API.

    By default, the class searches for the keys as environment variables.
    e.g. the keys have been exported by:
    ...
    twitter_bearer_token : str
        by default this is the env variable: 'TWITTER_BEARER_TOKEN'

    google_application_credentials : str
        by default this is the env variable: 'GOOGLE_APPLICATION_CREDENTIALS'

    verbose : bool
        should the code be verbose? By default this value is False.
    ...
    def check_twitter_connection(self):
        """
        The method checks the endpoints.
        If it can't connect it raisesan an exception.
        """
        _ = requests.request(
            "GET",
    ...
    def _bearer_oauth(self, r):
        """
        This uses the bearer token for authorization
...
class HashtagAnalyzer:
    """
    This class searches for tweets containing the specified hashtag
    within a specified range of time (not greater than the past 7 days)

    parameters
    ----------
    api_conns
        instantiation of the APIConnections class.

    hash_tag : str
        the hashtag to use. By default this variable is #Salem

    start_time : str
        start time of search. By default this is "2022-10-16T00:00:00Z"

    end_time : str
        end time of search. By default this is "2022-10-22T00:00:00Z"

    max_results : int
        number of results to return. By default this is 15

    no_retweets : bool
        should we exclude retweets? by default this is False (include retweets)
        If true, this will take the `max_results` tweets and reduce the number
        to those that are not RTs.

    verbose : bool
        print out information. By default this is false
    ...
    def analyze_tweets(self):
        """
        This analyze the tweets one-by-one, adds sentiment and magnitude scores
        to a copy of the original pd.DataFrame, and returns the dataframe.
    ...
    def _analyze_tweet(self, ind, tweet_text):
        """
        This method analyzes a tweet using Google NLP and returns a
        sentiment score and a magnitude score for each sentence in the tweet.
        """
        client = language_v1.LanguageServiceClient()
    ...
    def calculate_total_sentiment(self):
        ...
        (score, magnitude) : tuple
            a tuple consisting of the score and magnitude
        """
